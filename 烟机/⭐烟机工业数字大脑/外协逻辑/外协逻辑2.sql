with base_Outsourcing_tamp as
         ...
         select 
                ...
                ,GETDATE() etl_time
			--	into digital_brain_outsourcing_dashboard_index
			from base_Outsourcing_tamp
	...
	------------------------------------------------------- 
	  ...
	  with base_Outsourcing_tamp as
		...
		select 
           ...
           ,j_cnt		      -- 当月紧急订单
		   ,GETDATE() etl_time
		-- into digital_brain_outsourcing_dashboard_index_month
     from(		   
         select  EINDT_MONTH
		        ,count(1) as od_cont
