...
truncate table Report0624.dbo.r24_temp_base_ar;
...
-- 	   ,SUM(CLABS) as inv_quantity
--    INTO #inv_1005
--    FROM ODS_HANA.dbo.MCHB
--   WHERE WERKS = '2000' 
-- 	AND LGORT = '1005' --AND MATNR = '1BBE40104300'
--   GROUP BY MATNR;
...
WHERE KLART = '001' 
  AND ATINN = '0000000009';

----------------------------------------------

select SUPPLIER_DATE
	  ,ORDER_NUM
	 -- ,right('00000' + ltrim(ORDER_ITEM_NUM), 5) as ORDER_ITEM_NUM
...
----------------------------------------------
         insert into Report0624.dbo.r24_temp_base_ar
          select
                  a.require_no                                                       as require_no                       -- 预留号
                 ,a.require_line                                                     as require_line                     -- 预留号行号
                 ,a.wbs_code                                                         as wbs_code                         -- 替换 n.wbs_code WBS
			     ...
			     ,case when a.wbs_code ='Z-2000_XSCB004' then '上海中臣烟草机械配件有限责任公司' else  z.NAME1 end NAME1   -- 售达方
			     ,GETDATE()  etl_time
			 from #require_main  a
             join #wbs_info      n
        ...
        left join #require       h          
               on a.require_line        = h.require_line
              and a.require_no          = h.require_no
        left join Report0624.dbo.r24_stock_detail  stock
               on a.req_material_code   = stock.material_code
		left join #inv_qty i 
			   on a.req_material_code   = i.material_code
