truncate table report0624.dbo.r24_consolidate_table_ar
...
--		 ,midlgort
--	into #midlgort_1050
--	from Report0624.dbo.r24_1050_1051 a
--  where a.midlgort = 1050;
...
-------------------------------------------------------	
	...
	--		  a.MATNR  MATNR -- 物料编号
	--		 ,a.DELNR1 rsnum -- 预留号
	--		 ,a.DELPS1 rspos -- 预留号行号
	--		 ,a.MNG012 MNG01 -- 待交货数量 已审批/未审批
	--		 ,a.DELNR2 DELNR -- 计划协议号
	--		 ,a.DELPS2 DELPS -- 计划协议明细行号 
	--		 ,a.DELET2 DELET
	--		 ,a.DAT01  EINDT -- 计划协议交货行交货时间
	--	 into #delnr_55 -- #g
	--     from ODS_HANA.dbo.mrp_ww a
	--    where DELKZ1='AR'
   ...
   SELECT DISTINCT
			 ...
			 select a.MATNR
					,a.rsnum
					,a.rspos
					,case when b.STOCK_QTY_SM >= Cumulative_MNG01 then MNG01 else 0 end MNG01
					,a.DELNR
					,a.DELPS
					,a.DELET
					,a.EINDT
					,a.Cumulative_MNG01  STOCK_QTY
			   into #delnr_55
...
-------------------------------------------------------
 ...
 SELECT 
		...
		,update_date_time
		,update_date_time n45_update_date_time -- 当前节点时间
 ...
 from (
		 SELECT			
				...
				,CONVERT(VARCHAR, CAST(update_date_time AS DATETIME), 120) update_date_time	
...
-------------------------------------------------------
 ...
 insert into report0624.dbo.r24_consolidate_table_ar
         select
				 a.require_no
				,a.require_line
				,a.wbs_code
				...
				,m.n45_current_node_status  -- 物流状态
				,m.n45_update_date_time     -- 当前节点时间
				,p.CREATED_TS               -- 计划协议交货行同步时间
		   from  Report0624.dbo.r24_temp_base_ar a
	...
	-- left join #midlgort_1050 e 
	--		 on e.rsnum = a.require_no
	--		and e.rspos = a.require_line
	  ...
	  left join #logistics__status m
	       ...
	       and m.order_line_num = g.DELPS
	       and m.plan_item_num  = g.DELET
	  left join #sap_lp h 
			 on h.ebeln = g.delnr
	  ...
	  left join #plan_date p
			...
			and p.ORDER_ITEM_NUM    = g.DELPS
			and  CAST(CAST(p.DELIVERY_ITEM_NUM AS INT) AS NVARCHAR) = g.DELET;
