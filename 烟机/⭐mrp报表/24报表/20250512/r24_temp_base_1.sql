...
----------------------------------------------
       ...
       SELECT a.PSPNR   as wbs_code
             ,''        as wbs_desc
             ,a.DEL121  as assemble_order_no -- 装配订单号
             ,''        as zp_ltxt           -- 订单长文本
             ,a.BAUGR1  as material_code     -- 父项物料编码
             ,a.DAT001  as schedule_date     -- 订单交货期
             ,wbs.wbs_category_code
             ,wbs.wbs_category_desc
		into #wbs_info
        from ODS_HANA.dbo.mrp_ww as a
   left join YJ_DIM.dbo.dim_wbs as wbs
          on a.PSPNR = wbs.wbs_code
where a.DELKZ1='AR' 
  and wbs.last_version = 1
----------------------------------------------

  select  DELNR1 as require_no        -- 预留号                
		 ,DELPS1 as require_line      -- 预留号行号   
         ,MNG011 as require_quantity  -- 缺口数量
    into #quantity
...
----------------------------------------------
       ...
       select a.DELNR1   as require_no
		     ,a.DELPS1   as require_line
		     ...
		     ,cb.vendor_desc
			 ,cb.order_priority
      into #purchase_order_info
      from Report0624.dbo.r24_mrp_detail as a
 left join
     Report0624.dbo.r24_purchase_order_detail as cb
     on a.DELNR2 = cb.purchase_order_no
    and a.DELPS1 = '0' + cb.purchase_order_line
where a.DELKZ1='AR' 
  and a.DELKZ2 = 'BE'
  and a.MNG012  > 0
...
----------------------------------------------
   ...
   select a.DELNR1   as require_no
		 ,a.DELPS1   as require_line
	     ,sum(a.MNG012 ) as supply_quantity
	into #supply_quantity_sum
	from ODS_HANA.dbo.mrp_ww a
	where a.DELKZ1='AR' 
	  and a.DELKZ2 in ('PB', 'PA')
	  and a.MNG012  > 0
 group by DELNR1, DELPS1
...
----------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 left join #order_status as insp_order_status
