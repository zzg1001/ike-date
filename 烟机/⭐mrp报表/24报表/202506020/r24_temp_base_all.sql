...
truncate table Report0624.dbo.r24_temp_base_all;
...
--------------------库存--------------------------
  ...
  select 
          material_code
		,sum(inv_quantity) inv_quantity -- 1005库存量
		,sum(qty)   qty                 -- 1099库存量
		,sum(total) total_qty           -- 总库存量
		 INTO #inv_qty
	from( 
		 SELECT MATNR as material_code
			   ,SUM(case when LGORT = '1005' then CLABS else 0 end ) as inv_quantity
			   ,SUM(case when LGORT = '1099' then CLABS else 0 end ) as qty
			   ,sum(CLABS) total
		...
		union all 
		 ...
		 select 
			  ...
			  ,sum(case when LGORT = '1099' then SLABS else 0 end) as qty
			  ,sum(SLABS) total
...
----------------------------------------------
 ...
 select  A.ORDER_NUM  as order_code
		...
		,B.UPDATED_TS as order_sync_date
		,case C.STATUS
...
----------------------------------------------
         insert into Report0624.dbo.r24_temp_base_all
          select
                 ...
                 ,x.wbs_category_code                                                as wbs_category_code
                 ,x.wbs_category_desc                                                as wbs_category_desc
        ...
        left join #require       h          
               on a.req_material_code  = h.req_material_code 
